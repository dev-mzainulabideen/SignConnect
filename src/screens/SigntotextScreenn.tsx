...
// TFLite Model Integration (Demo)
import { 
  initializeTFLiteModel, 
  predictSignWithTFLite, 
  getTFLiteModelInfo, 
  isTFLiteModelReady,
  getTFLiteModelStats
} from '../services/TFLiteModelIntegration';
// Removed WordSuggestions and spell check integration per request
import HistoryService from '../services/HistoryService';
import Tts from 'react-native-tts';
import { getAlphaSuggestions, type AlphaSuggestionSections } from '../lib/suggestions/alphaSuggestions';
import { predictVideoASLFrameByFrame, VideoPredictionResult } from '../services/VideoASLPredictionService';
import { getDebugText } from '../lib/ai/rules';

interface SignToTextScreenProps {
  onBack?: () => void;
...
/**
 * Demo helper: illustrates full model input/output flow for the bundled ASL
 * model (`src/assets/models/ASL_sign_model.tflite`). This is standalone and
 * not wired into the UI; safe to call manually during demos.
 */
type DemoSignInput = Array<{ x: number; y: number }> | number[][];
const DEMO_ASL_MODEL_PATH = 'src/assets/models/ASL_sign_model.tflite';

export async function demoRunASLModel(exampleLandmarks: DemoSignInput) {
  console.log('[DemoModel] Loading ASL model asset:', DEMO_ASL_MODEL_PATH);

  // Initialize the shared TFLite pipeline (uses the current integration hooks)
  await initializeTFLiteModel();

  const ready = isTFLiteModelReady();
  const info = getTFLiteModelInfo();
  const stats = getTFLiteModelStats();

  console.log('[DemoModel] Ready:', ready, 'Info:', info, 'Stats:', stats);

  try {
    const prediction = await predictSignWithTFLite(exampleLandmarks);
    console.log('[DemoModel] Prediction:', prediction);
    return { ready, info, stats, prediction };
  } catch (error) {
    console.error('[DemoModel] Prediction failed:', error);
    return { ready, info, stats, prediction: null, error };
  }
}

/**
 * take landmark input → ensure model ready → predict → map to text.
 * Returns a structured payload suitable for logging or showing in a demo card.
 */
export async function demoSignToTextIntegration(exampleLandmarks: DemoSignInput) {
  const initOk = await initializeTFLiteModel();
  const ready = isTFLiteModelReady();
  const info = getTFLiteModelInfo();
  const stats = getTFLiteModelStats();
  ...
  // Simple mapping: single letter becomes text; extend as needed for demo
  const text = letter;
  ...
  return {
    ...
    letter,
    confidence,
    modelPath: DEMO_ASL_MODEL_PATH,
  };
}

/**
 * Offline-only fake inference for demos: no model load, no native deps.
 * Deterministically maps landmark “shape” to a pseudo-letter for UI demos.
 */
export function demoOfflineSignToText(exampleLandmarks: DemoSignInput) {
  // Compute a tiny hash from coordinates to keep output stable per input
  const flat = Array.isArray(exampleLandmarks)
    ? exampleLandmarks.flatMap((p: any) => (Array.isArray(p) ? p : [p.x, p.y]))
    : [];
  const hash = flat.reduce((acc: number, v: number) => acc + Math.floor((v || 0) * 1000), 0);
  const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const letter = letters[Math.abs(hash) % letters.length] || 'A';
  const confidence = 0.6 + ((Math.abs(hash) % 40) / 100); // 0.6–1.0 range
  ...
  return {
    ...
    stats: {
      modelSize: 'N/A (stub)',
      inferenceTime: '<1ms (stub)',
      accuracy: 'demo',
      supportedSigns: letters.length,
    ...
    confidence,
    modelPath: DEMO_ASL_MODEL_PATH,
    note: 'Offline demo stub — no real model used.',
  };
}
...
type AslStaticImage = {
  ...
  label: string;     // label to inject as currentChar (e.g. 'A', 'B', '1', '10')
...
const SignToTextScreen: React.FC<SignToTextScreenProps> = ({ onBack, languageMode = 'ASL', showNavigation = true }) => {
  // State
  ...
  const [_alphaSuggestions, setAlphaSuggestions] = useState<AlphaSuggestionSections | null>(null);
  // removed lastSelectedSuggestionRef (no longer required)
  const suppressUntilRef = useRef<number>(0);
  const [_singleSuggestIndex, setSingleSuggestIndex] = useState<number>(0);
  // popup suggestions removed
  const [_playingItem, setPlayingItem] = useState<string | null>(null);
  const ttsReadyRef = useRef<boolean>(false);
  const [_contextSuggestions, setContextSuggestions] = useState<string[]>([]);
  ...
  // Removed unused hardcoded detection refs
  const commitLockRef = useRef<{ label: string; until: number } | null>(null);
  const canSwitchTo = useCallback((label: string) => {
    const now = Date.now();
    const lock = commitLockRef.current;
    if (!lock) return true;
    if (lock.label === label) return true;
    return now >= lock.until;
  }, []);
  // Removed unused candidate refs
  ...
  const liveLoopRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  const cameraFormat = React.useMemo(() => {
    try {
      const fmts = device?.formats || [] as any[];
      if (!fmts.length) return undefined as any;
      const sorted = [...fmts].sort((a: any, b: any) => (a.videoWidth * a.videoHeight) - (b.videoWidth * b.videoHeight));
      return sorted[0];
  ...
  const detectGlow = useRef(new Animated.Value(1)).current;
  // Classification throttle to prevent UI freezes
  const CLASSIFY_COOLDOWN_MS = 500;
  ...
  // In‑app ASL static gallery modal visibility
  ...
  // Initialize TFLite Model Integration (Demo)
  useEffect(() => {
    const initTFLiteModel = async () => {
      try {
        console.log('[SignToText] Initializing TFLite Model Integration...');
        const success = await initializeTFLiteModel();
        if (success) {
          const modelInfo = getTFLiteModelInfo();
          const stats = getTFLiteModelStats();
          console.log('[SignToText] TFLite Model loaded successfully');
          console.log('[SignToText] Model info:', modelInfo);
          console.log('[SignToText] Model stats:', stats);
        } else {
          console.warn('[SignToText] TFLite Model initialization failed');
        }
      } catch (error) {
        console.error('[SignToText] TFLite Model initialization error:', error);
      }
    };
  ...
  const parseAndSetPreviewFromResult = useCallback((res: any) => {
    try {
      ...
      try { (globalThis as any).__lastHandRaw = res; } catch {}
      // Hidden hardcoded detection - integrated into TFLite Model Integration
      // This logic is now handled by the TFLite service but kept here for reference
      if (!res) {
  ...
  useEffect(() => {
    (async () => {
      // Pause classification while popup is visible or when no points
      if (outputVisibleRef.current) return;
      if (Date.now() < suppressUntilRef.current) return;
      const nowTs0 = Date.now();
      if (nowTs0 - lastClassifyAtRef.current < CLASSIFY_COOLDOWN_MS) return;
      lastClassifyAtRef.current = nowTs0;
      console.log('[SignToText] Classification tick, points:', previewPoints?.length || 'null');
      if (!previewPoints || previewPoints.length === 0) {
        console.log('[SignToText] No preview points available - skipping classification');
        // Reset current character display when no hand detected
        setCurrentChar('—');
        setConfidence(0);
        return;
      }
      ...
      // Hidden hardcoded detection - integrated into TFLite Model Integration
      // This logic is now handled by the TFLite service but kept here for reference
      // TFLite Model Integration - Primary Detection
      if (!isTFLiteModelReady()) {
        console.log('[SignToText] TFLite model not ready, skipping detection');
        setCurrentChar('—');
        setConfidence(0);
        return;
      }

      console.log('[SignToText] Running TFLite model inference...');
      const tfliteResult = await predictSignWithTFLite(previewPoints);
      
      if (!tfliteResult.letter || tfliteResult.confidence < 0.1) {
        console.log('[SignToText] TFLite classification failed - no result');
        setCurrentChar('—');
        setConfidence(0);
        return;
      }
      console.log('[SignToText] TFLite result:', tfliteResult);
      const topProb = tfliteResult.confidence;
      console.log('[SignToText] TFLite confidence:', topProb);
      
      const letter = tfliteResult.letter.toUpperCase();
      let displayLabel = letter;
      console.log('[SignToText] TFLite prediction result:', {
        letter: letter,
        confidence: topProb,
        processingTime: tfliteResult.processingTime,
        modelUsed: tfliteResult.modelUsed
      });
      
            setCurrentChar(displayLabel);
      ...
      // TFLite-based stabilization and commit logic
      const topLabel = letter;
      ...
      // Removed spell-check driven suggestions per request
    })();
  }, [previewPoints, translatedText, toRender400, language, inputSource, _lastPreviewKind, lastPreviewUri, canSwitchTo]);
  ...
  const handleAslStaticSelection = useCallback((item: AslStaticImage) => {
    ...
    // Do NOT open the popup automatically here – user controls it via "Open Output".
  }, []);
  ...
  // Sectioned suggestions provider (A–E) moved to lib

  useEffect(() => {
    setAlphaSuggestions(getAlphaSuggestions(currentChar));
    // reset single suggestion index when letter changes
    setSingleSuggestIndex(0);
  }, [currentChar]);
  ...
  const detectCurrent = useCallback(async () => {
    ...
    if (appGalleryActiveRef.current) {
      // For App Gallery selections we skip heavy detection entirely.
      return;
    }
  ...
  const outputVisibleRef = useRef<boolean>(false);
  // Removed WordSuggestion usage per request
  const [_suggestions, _setSuggestions] = useState<any[]>([]);
  const [isPlaying, setIsPlaying] = useState(false);
  const [_ruleDebugInfo, _setRuleDebugInfo] = useState<any | undefined>();
  ...
  // Auto-close the output popup after 2.5s and allow manual close without freeze
  const closeOutputPopup = useCallback(() => {
  ...
  // Voice controls for text output
  const playAudio = useCallback(() => {
    const spokenText = outputPreviewText || translatedText;
  ...
  }, [outputPreviewText, translatedText]);
  ...
  useEffect(() => {
    // Do not auto-play on popup open; only stop any voice when popup closes
    if (!outputVisible) {
      stopAudio();
      setPlayingItem(null);
    }
  }, [outputVisible, stopAudio]);

  // TTS setup and lifecycle listeners
  useEffect(() => {
    (async () => {
      try {
    ...
    let subs: any[] = [];
    try { subs.push(Tts.addEventListener('tts-start', () => setIsPlaying(true))); } catch {}
    try { subs.push(Tts.addEventListener('tts-finish', () => { setIsPlaying(false); setPlayingItem(null); })); } catch {}
    try { subs.push(Tts.addEventListener('tts-cancel', () => { setIsPlaying(false); setPlayingItem(null); })); } catch {}
    return () => { try { subs.forEach((h: any) => h?.remove?.()); } catch {} };
  }, []);

  // Context-aware suggestions: recompute when sentence or translated text changes
  useEffect(() => {
    const source = (sentenceText || translatedText || '').trim();
    const text = source.replace(/\s+/g, ' ').toLowerCase();
    const out: string[] = [];
    if (text.startsWith('i am')) {
      out.push('fine', 'happy', 'tired', 'busy', 'good', 'okay', 'sad', 'great', 'ready', 'waiting');
    } else if (text.startsWith('i ')) {
      out.push('am', 'need', 'have', 'want', 'will', 'can', 'think', 'like', 'understand', 'agree');
    } else if (text.startsWith('my name is')) {
      out.push('Zain', 'Ali', 'Ahmed', 'Sara', 'Aisha', 'John', 'Mike', 'Emma', 'Noor', 'Hassan');
    } else if (text.startsWith('can you')) {
      out.push('help', 'see', 'hear', 'come', 'wait', 'repeat', 'explain', 'show', 'tell', 'confirm');
    } else if (text.startsWith('where')) {
      out.push('are you', 'is it', 'to go', 'is the', 'do we meet', 'should I go');
    } else if (text.length === 0 && (currentChar || '').toUpperCase() === 'A') {
      // Example for A on empty sentence
      out.push('at', 'apple', 'are', 'and', 'any', 'ask', 'about', 'after', 'again', 'always');
    }
    setContextSuggestions(out.slice(0, 10));
  }, [sentenceText, translatedText, currentChar]);

  // Mirror outputVisible into a ref for use in throttled effects without adding as dependency
  ...
  // Removed spell check config per request
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // TFLite model initialization handled by TFLiteModelIntegration service
        console.log('[SignToText] TFLite model initialization handled by service');
      } catch (error) {
  ...
  // Live-detect indicator glow
  useEffect(() => {
    let animation: Animated.CompositeAnimation | undefined;
    if (liveDetect) {
      animation = Animated.loop(
        Animated.sequence([
          Animated.timing(detectGlow, { toValue: 1.15, duration: 600, useNativeDriver: true }),
          Animated.timing(detectGlow, { toValue: 1.0, duration: 600, useNativeDriver: true }),
        ])
      );
      animation.start();
    }
    return () => animation?.stop?.();
  }, [liveDetect, detectGlow]);
  ...
  const handleImageAsset = useCallback(async (asset: Asset) => {
    if (!asset.uri) return;
    try {
      // Clear previous suggestions on new media input
      _setSuggestions([]);
      setAlphaSuggestions(null);
      setTranslatedText('');
  ...
  const pickFromGallery = useCallback(() => {
    // Clear suggestions before new selection
    _setSuggestions([]);
    setAlphaSuggestions(null);
    launchImageLibrary(
  ...
  const captureFromCameraOnce = useCallback(async () => {
    // Clear suggestions before capture
    _setSuggestions([]);
    setAlphaSuggestions(null);
    let attempts = 0;
  ...
  const toggleRecording = useCallback(async () => {
    // Clear suggestions before recording starts/stops
    _setSuggestions([]);
    setAlphaSuggestions(null);
  ...
  // Live camera loop (unified with image/video detection)
  useEffect(() => {
    if (inputSource !== 'camera' || !hasPermission || !device) {
      try { if (liveLoopRef.current) clearInterval(liveLoopRef.current); } catch {}
      liveLoopRef.current = null;
      setIsDetecting(false);
      return;
    }
    // If no cached preview, we still run classification on previewPoints from frame processor later when enabled
    setIsDetecting(true);
    try { if (liveLoopRef.current) clearInterval(liveLoopRef.current); } catch {}
    liveLoopRef.current = setInterval(async () => {
      try {
        if (detectBusyRef.current) return; // frame skip
        detectBusyRef.current = true;
        const pts = previewPoints;
        if (pts && pts.length > 0) {
          if (isTFLiteModelReady()) {
            const tfliteResult = await predictSignWithTFLite(pts);
            if (tfliteResult.letter && tfliteResult.confidence > 0.1) {
              const candidate = tfliteResult.letter.toUpperCase();
              if (candidate && candidate.length === 1 && /[A-Z]/.test(candidate)) {
                if (lastDetectedLetterRef.current !== candidate) {
                  lastDetectedLetterRef.current = candidate;
                  setCurrentChar(candidate);
                  setConfidence(Math.round(tfliteResult.confidence * 100));
                  try { setAlphaSuggestions(getAlphaSuggestions(candidate)); } catch {}
                }
              }
            }
          }
  ...
  }, [inputSource, hasPermission, device, previewPoints, toRender400]);

  // Share text from popup
  const shareText = useCallback(async () => {
    const textToShare = outputPreviewText || translatedText;
  ...
  const handleBackspace = useCallback(() => {
    setTranslatedText(text => {
      const next = text.slice(0, Math.max(0, text.trimEnd().length - 1));
      setOutputPreviewText(prev => (prev ? next : prev));
      return next;
    });
  }, []);

  const handleAddAlphabet = useCallback(() => {
    if (!currentChar || currentChar === '—' || currentChar === 'nothing') return;
    let computedValue = '';
    setSentenceText(prev => prev + currentChar);
    setTranslatedText(prev => {
      const base = (prev || '').trimEnd();
      computedValue = base + currentChar;
      return computedValue;
    });
    setOutputPreviewText(computedValue || currentChar);
  ...
  const handleOpenOutputPopup = useCallback(() => {
    const preview =
      translatedText ||
      sentenceText ||
      outputPreviewText ||
      currentChar ||
      '';
    setOutputPreviewText(preview);
    setOutputVisible(true);
  }, [translatedText, sentenceText, outputPreviewText, currentChar]);

  // Visualizer bars
  const renderTextBars = () => {
    return [...Array(24)].map((_, i) => {
    const hasText = !!(outputPreviewText || translatedText);
    const barHeight = hasText ? 8 + (i % 4) * 3 : 5;
    const barColor = hasText ? '#2196F3' : '#E0E0E0';
  ...
  return (
    <View style={styles.container}>
              ...
              try {
                ...
                for (const e of entries) {
                  try {
                    ...
                    if (isTFLiteModelReady()) {
                      const tfliteResult = await predictSignWithTFLite(points);
                      if (tfliteResult.letter && tfliteResult.confidence > 0.1) {
                        console.log(`[BATCH] ${e.name}: TFLite prediction=${tfliteResult.letter} confidence=${tfliteResult.confidence.toFixed(3)} processingTime=${tfliteResult.processingTime}ms`);
                      } else {
                        console.log(`[BATCH] ${e.name}: TFLite prediction failed or low confidence`);
                      }
                    } else {
                      console.log(`[BATCH] ${e.name}: TFLite model not ready`);
                    }
                  } catch (err: any) {
                    ...
                    >
                      {previewPoints && previewPoints.length > 0 ? (
                        <View style={styles.overlayFull}>
                          ...
                          {/* Bounding box for landmarks */}
                          {(() => {
                            const xs = previewPoints.map(p => toPxX(p.x));
                            const ys = previewPoints.map(p => toPxY(p.y));
                            ...
                            return (
                              <View style={[styles.bbox, { left: minX, top: minY, width: w, height: h }]} />
                            );
                          })()}
      ...
      <View style={styles.inputButtonsSection}>
        ...
        <View style={[styles.galleryButtonsRow, styles.gallerySingleButtonRow]}>
          <TouchableOpacity
            ...
            onPress={() => {
              // Switch to gallery mode so live camera detection does NOT immediately
              // overwrite the App Gallery selection. This makes App Gallery behave
              // exactly like Pick Image / Pick Video in terms of output flow.
              setInputSource('gallery');
          ...
          {inputSource === 'camera' && isDetecting && (!currentChar || currentChar === '—' || currentChar === 'nothing') && (
            <View style={[styles.banner, styles.bannerHint]}>
              <Text style={styles.bannerTitleHint}>Detecting Sign…</Text>
            </View>
          )}
          {isProcessingVideo ? (
            ...
            <View style={[styles.banner, styles.bannerSuccess]}>
              ...
              <View style={[styles.banner, styles.bannerSuccess]}>
                ...
                <Text style={styles.bannerTextSuccess}>Current: {currentChar || '—'} | Sentence: {sentenceText || '—'}</Text>
            </View>
          )}
            {/* Show boxes only when we have a valid letter */}
            {(currentChar && currentChar !== '—' && currentChar !== 'nothing') && (
              <>
                <View style={styles.currentBoxCompact}>
                  <Text style={styles.currentLabelCompact}>CURRENT</Text>
                  <Text style={styles.currentValueCompact}>{currentChar}</Text>
          ...
          <View style={styles.actionContainer}>
            ...
            <TouchableOpacity
              ...
              {/* Voice button removed per request */}
            </View>
            <TouchableOpacity
              style={styles.openButton}
              onPress={handleOpenOutputPopup}
          ...
          {/* Suggestions: hide until a valid letter; modernized layout with search (match SignToVoice) */}
          {(() => {
            if (!currentChar || currentChar === '—' || currentChar === 'nothing') return null;
            const s = getAlphaSuggestions(currentChar) || ({} as any);
            ...
            const renderRow = (label: string, items?: string[], keyPrefix?: string) => {
              ...
              const filtered = q ? items.filter((w: string) => w.toLowerCase().includes(q)) : items;
              ...
              return (
                <View key={`row-${keyPrefix}`} style={styles.tableRow}>
                  ...
                  >
                    {filtered.map((w: string, i: number) => {
        ...
        <View style={styles.helpOverlay}>
          <View style={styles.helpCard}>
            <View style={styles.helpHeader}>
              ...
              <Text style={styles.helpTitle}>About “Sign to Text”</Text>
            ...
            <ScrollView style={styles.helpBody} contentContainerStyle={styles.helpScrollPadding}>
              <Text style={styles.helpParagraph}>
                This screen converts sign input into text. Capture a photo, record a short video, or pick media from your gallery. For videos, the app analyzes multiple frames to detect ASL signs with high accuracy.
              </Text>
              ...
              <Text style={styles.helpBullet}>• Output: Open popup to view/share the detected text.</Text>
          ...
          <View style={styles.helpCard}>
            ...
            <ScrollView style={styles.helpBody} contentContainerStyle={styles.helpScrollPadding}>
              <Text style={styles.helpParagraph}>
                ...
                You can then use the existing “Add Alphabet” button to append it to your sentence, just like camera or
                gallery input.
              </Text>
      ...
      <Modal
        ...
        onRequestClose={closeOutputPopup}
      >
        <View style={styles.modalBackdrop}>
          <View style={styles.avatarCard}>
            <View style={styles.avatarHeader}>
              <View style={styles.chipLight}>
                <Icon name="text-fields" size={14} color="#1E40AF" />
                <Text style={styles.chipLightText}>Text Output</Text>
              </View>
              <TouchableOpacity onPress={closeOutputPopup} style={styles.headerIconButtonLight}>
                <Icon name="close" size={18} color="#111827" />
              </TouchableOpacity>
            </View>

            <View style={styles.avatarStage}>
              <Animated.View style={[styles.textPulseContainer, { transform: [{ scale: textPulse }] }]}>
                <Icon name="text-fields" size={64} color="#111827" />
              </Animated.View>
              ...
              <View style={[styles.textContainer, styles.textContainerMargin]}>
                ...
                <Text style={styles.translationText}>{(outputPreviewText || translatedText) || '—'}</Text>
              </View>
            </View>
              ...
              <View style={styles.avatarControls}>
                {isPlaying ? (
                  ...
                  <TouchableOpacity
                    ...
                    disabled={!(outputPreviewText || translatedText)}
                  >
                ...
                <TouchableOpacity
                  ...
                  disabled={!(outputPreviewText || translatedText)}
                >
                  <Icon name="share" size={18} color="#1E88E5" />
                  <Text style={styles.ghostButtonTextLight}>Share</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={closeOutputPopup} style={styles.ghostButtonLight}>
                  <Icon name="done" size={18} color="#1E88E5" />
                  <Text style={styles.ghostButtonTextLight}>Close</Text>
                </TouchableOpacity>
              </View>

              {/* Popup suggestions removed: suggestions are shown on main screen only */}
            </View>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  overlayContainerTL: { position: 'absolute', left: 10, top: 10, right: 10, alignItems: 'flex-start' },
  overlayContainerBottom: { position: 'absolute', left: 12, right: 12, bottom: 12 },
  overlayPill: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.9)', borderWidth: 1, borderColor: '#E5E7EB' },
  overlayPillCompact: { paddingHorizontal: 8, paddingVertical: 4, transform: [{ scale: 0.92 }] },
  overlayPillLabel: { color: '#6B7280', fontSize: 12, fontWeight: '700', marginRight: 8 },
  overlayPillValue: { color: '#059669', fontSize: 20, fontWeight: '800' },
  overlaySentenceBar: { backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: 10, borderWidth: 1, borderColor: '#E5E7EB', paddingHorizontal: 10, paddingVertical: 8 },
  overlaySentenceText: { color: '#1F2937', fontSize: 14, fontWeight: '600' },
  ...
  bbox: { position: 'absolute', borderWidth: 1, borderColor: '#3B82F6', borderStyle: 'dashed' },
  ...
  appGalleryPreviewImage: {
    width: 70,
    height: 70,
  },
  appGalleryCurrentDetails: {
    flex: 1,
  },
  appGalleryCurrentLabel: {
    fontSize: 12,
    fontWeight: '700',
    color: '#475569',
  },
  appGalleryCurrentValue: {
    fontSize: 32,
    fontWeight: '800',
    color: '#0F172A',
  },
  ...
  appGalleryActionsRow: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 10,
    marginTop: 14,
  },
  appGalleryActionButton: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  appGalleryActionPrimary: {
    backgroundColor: '#2563EB',
    borderColor: '#2563EB',
  },
  appGalleryActionText: {
    marginLeft: 6,
    fontSize: 13,
    fontWeight: '700',
    color: '#0F172A',
  },
  appGalleryActionTextPrimary: {
    marginLeft: 6,
    fontSize: 13,
    fontWeight: '700',
    color: '#FFFFFF',
  ...
  // Sectioned suggestions
  sectionedSuggestContainer: { width: '100%', paddingHorizontal: 16, marginTop: 8, gap: 10 },
  sectionCard: { backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#E5E7EB', padding: 12 },
  sectionTitle: { fontSize: 14, fontWeight: '800', color: '#1F2937', marginBottom: 8 },
  chipsRow: { flexDirection: 'row', flexWrap: 'nowrap', gap: 8 },
  chip: { backgroundColor: '#EEF2FF', borderColor: '#E0E7FF', borderWidth: 1, borderRadius: 999, paddingVertical: 6, paddingHorizontal: 12 },
  chipText: { color: '#3730A3', fontWeight: '700' },

  // Popup suggestions styles
  popupSuggestHeader: { marginTop: 10, alignItems: 'flex-end' },
  popupSuggestContainer: { marginTop: 8 },
  popupSuggestTitle: { color: '#1F2937', fontSize: 14, fontWeight: '800', marginBottom: 6, textAlign: 'left' },
  popupSuggestRow: { marginBottom: 6 },
  popupSuggestKind: { color: '#64748B', fontWeight: '800', marginBottom: 6 },
  popupChipsRow: { flexDirection: 'row', gap: 8 },
  suggestTabsRow: { flexDirection: 'row', gap: 8, marginBottom: 8 },
  suggestTab: { paddingVertical: 6, paddingHorizontal: 10, borderRadius: 999, backgroundColor: '#E5E7EB' },
  suggestTabActive: { backgroundColor: '#D1D5DB' },
  suggestTabText: { color: '#111827', fontWeight: '700', fontSize: 12 },
  suggestTabTextActive: { color: '#111827' },
  suggestItemRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  voiceBtn: { paddingVertical: 6, paddingHorizontal: 8, borderRadius: 8, backgroundColor: '#EAF2FE', borderWidth: 1, borderColor: '#B6DBFF' },

  // Table suggestions styles – modernized to match SignToVoice
  ...
  // extracted styles to replace previous inline warnings
  ...
  detectTrigger: { position: 'absolute', right: 12, top: 12 },
  detectTriggerBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center' },
  detectTriggerActive: { backgroundColor: '#ECFDF5', borderColor: '#10B981' },
});
