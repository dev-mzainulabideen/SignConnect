...
/**
 * Demo-only model stub for sign-to-sign: illustrates how we'd invoke a bundled
 * TFLite model (`src/assets/models/Sign-to-sign_model.tflite`) to map an input
 * video (URI or asset) to an output sign label. This does NOT load or run a
 * real model—kept offline and deterministic for demos without native deps.
 */
const DEMO_SIGN_TO_SIGN_MODEL = 'src/assets/models/Sign-to-sign_model.tflite';

type DemoVideoInput =
  | { uri: string; type?: 'video/mp4' | string }
  | number // require() id
  | string; // local path

export function demoRunSignToSignModel(inputVideo: DemoVideoInput) {
  // Stable hash from input to pick a pseudo label
  const keySeed =
    typeof inputVideo === 'number'
      ? inputVideo
      : typeof inputVideo === 'string'
      ? inputVideo.length
      : (inputVideo?.uri || '').length;

  const labels = Object.keys(PSL_OUTPUT_BY_KEY).concat(Object.keys(ASL_OUTPUT_BY_KEY));
  const label = labels.length
    ? labels[Math.abs(keySeed) % labels.length]
    : 'hello';

  return {
    modelPath: DEMO_SIGN_TO_SIGN_MODEL,
    ready: true,
    label,
    confidence: 0.75,
    info: {
    ...
    stats: {
      ...
      supportedLabels: labels.length || 'N/A',
    },
    note: 'Offline demo stub — no real model execution.',
  };
}
...
const SignToSignScreen: React.FC<SignToSignScreenProps> = ({ onBack, languageMode = 'ASL', showNavigation = true }) => {
  ...
  // Simulated on-device model inference for gallery-picked videos (demo-only)
  useEffect(() => {
    if (!selectedInputVideo) return;
    const inference = demoRunSignToSignModel(
      (selectedInputVideo as any)?.source ||
        (selectedInputVideo as any)?.fileName ||
        selectedWordKey ||
        'video',
    );
    const label = inference.label || '';
    if (label) {
      setRecognizedLabel(label);
      setTranslatedLabel(label);
      const detected = detectLanguageOfLabel(label);
      setSourceLanguage(detected);
      setTargetLanguage(detected === 'ASL' ? 'PSL' : 'ASL');
    }
  }, [selectedInputVideo, selectedWordKey, detectLanguageOfLabel]);
